...
public class RestaurantReviewsModel {
	...
	public ObservableList<Review> getReviewsOfMenuItem(int menuItemId) {
		...
		try (Connection conn = Database.getConnection()) {
			String statement = "select Review.review_id, User.full_name, Review.rating, Review.comment, Review.date_submitted from Review inner join User on Review.user_id = User.user_id where Review.menu_item_id = ? order by Review.date_submitted desc, Review.review_id desc";
